...
from email import quoprimime, contentmanager
import re
import sys
import time
import random
...
class Generator:
    ...
    def _handle_message_global(self, msg):
        s = self._new_buffer()
        g = self.clone(s)
        # Always treat message/global as UTF-8 then encode it later if
        # necessary:
        g.policy = g.policy.clone(utf8=True)
        # The payload of a message/global part should be a multipart sequence
        ...
        if msg['Content-Transfer-Encoding'] == 'base64':
            payload = contentmanager._encode_base64(payload, self.policy.max_line_length)
        else:  # Treat as quoted-printable by default.
            payload = quoprimime.encode(payload.encode('utf-8'),
                                             self.policy.max_line_length).decode('ascii')
